...
Public Sub GetListData()
    ...
    If ArchiveName = "" Then
        frmMain.StatusBar.Panels(1).Text = "Archive *"
            Else
        frmMain.StatusBar.Panels(1).Text = "Archive (" & ArchiveName & ")"
    End If
    
    frmMain.StatusBar.Panels(5).Text = "Sel file num " & frmMain.ListFiles.SelectedItem.Index
    
    If frmMain.ListFiles.SelectedItem.SubItems(1) <> "" Then
        frmMain.StatusBar.Panels(2).Text = "Offset (" & frmMain.ListFiles.SelectedItem.SubItems(4) & ")"
            Else
        frmMain.StatusBar.Panels(2).Text = "Offset *"
    End If
    
    If frmMain.ListFiles.SelectedItem.SubItems(2) <> "" Then
        frmMain.StatusBar.Panels(3).Text = "File size (" & frmMain.ListFiles.SelectedItem.SubItems(2) & ")"
            Else
        frmMain.StatusBar.Panels(3).Text = "File size *"
    End If
    
    If ArchiveName = "" Then
        frmMain.Caption = "CyberCrypt (No new file)"
    ElseIf ArchiveName <> "" Then
        frmMain.Caption = "CyberCrypt (" & ArchiveName & ")"
    End If
...
Public Function KillSf(Filen As String) As Boolean
    On Error GoTo FinaliseError
    Close
    Kill Filen
    KillSf = True
